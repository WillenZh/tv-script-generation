...
def create_lookup_tables(text):
    ...
    vocab_to_int = {}
    int_to_vocab = {}
    for ii, word in enumerate(set(text), 1):
        #print("{},{}".format(ii, word))
...
def token_lookup():
    ...
    """
    # TODO: Implement Function
    return {'--':'||dash||', '.':'||period||', ',':'||comma||', 
            '"':'||quotation_mark||', ';':'||semicolon||', '!':'||exclamation_mark||', '?':'||question_mark||',
...
def get_inputs():
    ...
    """
    # TODO: Implement Function
    return tf.placeholder(tf.int32, [None, None], 'input'), tf.placeholder(tf.int32, [None, None]), tf.placeholder(tf.float32)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    for n in range(0, int_text.shape[1], seq_length):
        ...
        #print(y[:, :].shape)
        #print("\n\n")
        #print(type(int_text[:, 1+n:1+n+seq_length]))
        
        if(1+n+seq_length <= int_text.shape[1]):
            y[:, :] = int_text[:, 1+n:1+n+seq_length]
        else:
            y[:, :-1] = int_text[:, 1+n:n+seq_length]
            y[:batch_size-1, -1] = int_text[1:, 0]
            y[batch_size-1, -1] = int_text[0, 0]
